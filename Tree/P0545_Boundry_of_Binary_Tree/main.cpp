...
public:
    vector<int> boundaryOfBinaryTree(TreeNode* root) {
        ...
        // Bottom Boundry -- preorder traversal
        vector<int> bottom;
        getBottom(root, bottom);
        ...
        // Right Boundry
        x = root->right;
        vector<int> rightBoundry;
        while (x != nullptr) {
            if (x->left != nullptr || x->right != nullptr)
                rightBoundry.push_back(x->val);
            if (x->right != nullptr) x = x->right;
            else x = x->left;
        }
        for (int i = rightBoundry.size() - 1; i >= 0; i--) {
            res.push_back(rightBoundry[i]);
        }

        return res;
    }

    void getBottom(TreeNode* root, vector<int>& bottom) {
        if (root != nullptr) {
